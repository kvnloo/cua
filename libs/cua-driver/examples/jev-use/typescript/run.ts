...
async function run(args: Arguments): Promise<Outcome> {
  ...
  async function writeEvent(path: string | undefined, event: Record<string, unknown>) {
    const line = JSON.stringify(
      event.event === 'outcome'
        ? { ...event, observation_ledger: ledger.entries() }
        : event,
    );
    console.log(line);
  ...
  try {
    ...
    for (let step = 1; step <= args.maxSteps; step += 1) {
      ...
      // parse_visual_regions) is only consumed by the visual-submit fallback
      // in buildCandidates. When the snapshot already yields an actionable
      // candidate, paying for it is pure overhead — capture only the needed
      // modality (#3963).
          ...
          ledger.record(
            {
              ...
              latencyMs: Math.round((performance.now() - visualStarted) * 100) / 100,
            },
            step,
          );
